...
import { INIT_TRANSFER, TRANSFER_ERROR, TRANSFER_SUCCESS } from './mutations';
...
export const actions = {
  [POST_TRANSFER]: async ({ commit, state, getters, dispatch }, { transaction, password }) => {
    // console.log('POST_TRANSFER :: ', state);
    try {
      commit(INIT_TRANSFER);
      //
      const response = await services.auth.fetchAccount(state.wallets.current.address);
      ...
      if (response.data.result.value) {
        ...
        let key;
        var bytes = CryptoJS.AES.decrypt(state.wallets.current.privatekey, password);
        ...
        // const responsePostTransfer = await services.transfer?.postTransfer(bcTransactionme);
        const responsePostTransfer = {};
