    ...
    <div class="basic-form modal-body">
      <div class="modal-header">
        <h5 class="modal-title">
          Withdraw from
          <strong :style="{ fontWeight: '600' }">{{ validator.description.moniker }}</strong>
        </h5>
      </div>
        ...
        <ul>
          <li>
            ...
            <select v-model="withdraw.config">
              <option key="0" value="0">Rewards only</option>
              <option key="1" value="1">Commisions only</option>
              <option key="2" value="2">Rewards and commissions</option>
            </select>
            <span v-if="withdraw.config == 1 || withdraw.config == 2" class="local-alert">{{
              $t('withdraw_with_commission_alert')
            }}</span>
          </li>
        </ul>
        <div v-if="!multisig">
        ...
        <b-row align-v="center" align-h="center">
          <b-col class="text-center">
            <div v-if="!tx.loading">
              ...
              <span v-else>
                <a v-if="withdraw.output == ''" class="btn btn-primary" @click="sendWithdrawTx"> Generate </a>
  ...
  methods: {
    ...
    download() {
      ...
      return util.download(
        date_today + '_withdraw_from_' + this.validator.description.moniker + '.json',
        document,
