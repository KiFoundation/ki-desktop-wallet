<template>
  <div class="d-flex w-100 h-100 flex-column px-3">
    ...
    <div v-if="validators && validators.length" class="mt-3 d-flex flex-column justify-content-between h-100">
      <div>
        <b-row class="header align-items-center mx-0" style="min-height: 40px">
          <b-col>
            <b-row class="w-100 align-items-center">
              ...
              <b-col cols="4"> Pending reward </b-col>
        ...
        <ValidatorCard
          ...
          :key="`validator-${idx}`"
          :validator="validator"
          :reward="(rewards && rewards[validator.operator_address]) || 0"
          class="mb-2"
        >
          <a v-b-modal="'withdraw-modal'" class="link" @click="selectValidator(validator)"> Withdraw </a>
        </ValidatorCard>
        <TotalCard :key="`total`" :reward="mergedRewards" class="mb-2">
          <a v-b-modal="'withdraw-all-modal'" class="link"> Withdraw all </a>
        </TotalCard>
      </div>
      <div
        class="mt-2 mb-2"
    ...
    <div v-if="selectedValidator">
      <WithdrawModal :validator="selectedValidator" @onWithdrawSuccess="handleWithdrawSuccess" />
    </div>
    <WithdrawAllModal
...
import { BRow, BCol, BPagination, BFormInput } from 'bootstrap-vue';
...
import TotalCard from '@cmp/withdraw/total.card.vue';
import WithdrawModal from './modals/withdraw.modal.vue';
import WithdrawAllModal from './modals/withdraw-all.modal.vue';
