<template>
  <div
    id="sign-form"
    class="d-flex w-100 h-100 flex-column px-3"
  ...
  >
    <form v-if="sign.file_valid" class="basic-form modal-body" style="padding-top: 40px">
      <b-row>
            ...
            <b-col cols="12">
              <div class="buttonInside">
                ...
                <a class="inside" @click="removeFile()"
                  ><img src="/img/icons/delete.png" style="width: 25px; opacity: 0.2"
                /></a>
              </div>
            </b-col>
          </b-row>
        ...
        <b-col cols="6" v-if="sign.onbehalf != account.id && !signRawData">
          <b-row style="margin-bottom: 10px">
            <b-col cols="10">
              <h6>
                {{ $t('webwallet_sign_onbehalf') }}
                <span style="font-weight: 800"> {{ sign.onbehalf_name }} </span>
              </h6>
            </b-col>
            <b-col />
          </b-row>
      ...
      <b-row style="margin-bottom: 25px">
        <b-col cols="10">
          <div class="buttonInside">
            <input v-model="wallet_pass_tmp" :type="password" :class="[wallet_pass_tmp ? '' : sign.alert]" />
        ...
        <b-col cols="2">
          ...
          <a v-else class="btn btn-download mt-1" @click="downloadSig">{{ $t('download') }}</a>
        </b-col>
      </b-row>
    ...
    <form v-else>
      ...
      <div class="upload-form">
        <b-row align-v="center">
          <b-col cols="4" />
          <b-col>
            <div v-cloak ref="myFile" class="upload-area" @drop.prevent="upload" @dragover.prevent>
              <p>
                <img src="/img/icons/add.png" style="width: 100px; opacity: 0.2; margin-bottom: 20px" />
              </p>
              <span v-if="signRawData" style="opacity: 0.3">{{ $t('webwallet_drag_drop_data') }}</span>
              <span v-else style="opacity: 0.3">{{ $t('webwallet_drag_drop') }}</span>
            </div>
          </b-col>
          <b-col cols="4" />
        </b-row>
...
import { Secp256k1Wallet, encodeSecp256k1Pubkey } from '@cosmjs/amino';
  ...
  methods: {
          ...
          switch (msg_[0].type) {
            case 'cosmos-sdk/MsgSend':
              ...
              return (
                ...
                '\t\t(' +
                this.findWalletName(msg.value.from_address) +
                ')' +
                ' \nto:\t\t ' +
                msg.value.to_address +
                '\t\t\t(' +
                this.findWalletName(msg.value.to_address) +
                ')'
              );
              break;
            ...
            case 'cosmos-sdk/MsgDelegate':
              ...
              return (
                ...
                '\t\t(' +
                this.findWalletName(msg.value.delegator_address) +
                ')' +
                '\nto:\t\t\t ' +
    ...
    downloadSig() {
      return util.download(
        this.sign.file.name.replace('.json', '') + '_signed_' + this.account.name + '.json',
        document,
        this.sign.signature
      );
    },
      ...
      if (!this.signRawData) {
        if (transaction.msg.length == 1) {
          ...
          if (response.data.result.value) {
            let res = '';
            if (
              response.data.result.type == 'cosmos-sdk/ContinuousVestingAccount' ||
              response.data.result.type == 'cosmos-sdk/DelayedVestingAccount'
            ) {
          ...
          const fees =
            transaction.fee.amount[0].amount === '0'
              ? { amount: [], gas: transaction.fee.gas }
              : transaction.fee;
          signingInstruction = {
