      ...
      <form class="basic-form">
        <li class="token">
          ...
          <datalist id="validator_unbond_list">
            <option v-for="(item, index) in validators" :key="index" :value="item.operator_address">
        ...
        <ul class="basic-group clearfix">
          ...
          <li class="token">
            <label>Token</label>
            <input v-model="redelegate.token" type="text" placeholder="0" disabled />
          </li>
        </ul>
  ...
  methods: {
    ...
    download() {
      ...
      return util.download(
        date_today + '_redelegate_' + this.redelegate.amount + 'xki' + '.json',
        document,
