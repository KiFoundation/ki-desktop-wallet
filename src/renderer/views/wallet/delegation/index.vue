<template>
  <div class="d-flex w-100 h-100 flex-column px-3">
    <div class="d-flex justify-content-between align-items-end mt-1" :style="{ position: 'relative' }">
      <div class="h-100">
        <b-form-input
    ...
    <div v-if="validators && validators.length" class="mt-3 d-flex flex-column justify-content-between h-100">
      <div>
        <b-row class="header align-items-center mx-0" style="min-height: 40px">
          <b-col>
            <b-row class="w-100 align-items-center">
              ...
              <b-col cols="4"> Share / total </b-col>
        ...
        >
          <div>
            <a v-b-modal="'delegate-modal'" class="link ml-2" @click="selectedValidator = validator">
              Delegate
            </a>
            <a
      ...
      >
        <b-pagination
          v-model="currentPage"
          :total-rows="
            validators.filter((v) => new RegExp(`^${text}`, 'gi').test(v.description.moniker)).length
          "
          :per-page="perPage"
    ...
    <div v-if="selectedValidator">
      <DelegateModal :validator="selectedValidator" @onDelegateSuccess="handleDelegateSuccess" />
      <UndelegateModal :validator="selectedValidator" @onUndelegateSuccess="handleUndelegateSuccess" />
      <RedelegateModal :validator="selectedValidator" @onRedelegateSuccess="handleRedelegateSuccess" />
    </div>
  </div>
</template>
...
import { BRow, BCol, BButton, BButtonGroup, BPagination, BFormInput } from 'bootstrap-vue';
import { FETCH_WALLET_VALIDATORS } from '@store/wallets';
import { FETCH_VALIDATORS_LIST } from '@store/validators';
import ValidatorCard from '@cmp/validator/validator.card.vue';
import DelegateModal from './modals/delegate.modal.vue';
import UndelegateModal from './modals/undelegate.modal.vue';
import RedelegateModal from './modals/redelegate.modal.vue';
