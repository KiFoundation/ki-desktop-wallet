    ...
    <form v-if="multisign.file_valid" class="basic-form modal-body" style="padding-top: 40px">
      <b-row>
        ...
        <b-col cols="6">
          ...
          <b-row style="margin-bottom: 20px">
            <b-col cols="12" style="max-height: 120px">
              <div class="wallet-list" style="max-height: 100%">
                <b-table
                  style="font-size: 11px; text-align: left; margin-bottom: 0px"
                  sticky-header
                  no-border-collapse
                  hover
                  borderless
          ...
          <b-row style="margin-bottom: 10px">
            <b-col cols="4">
              <h6>{{ $t('webwallet_sign_threshold') }}</h6>
            </b-col>
          ...
          <b-row align-v="end">
            <b-col>
              <b-progress :max="pubkeys.length" height="25px">
                ...
                <b-progress-bar
                  :value="threshold - multisign.progress"
          ...
          <b-row>
            <b-col cols="6">
              <div
                ...
                ref="myFile"
              >
                <p><img src="/img/icons/add.png" style="width: 70px; opacity: 0.2; margin-bottom: 20px" /></p>
              ...
              <div class="wallet-list" style="max-height: 100%; margin-bottom: 0">
                <li class="token" v-for="(item, index) in multisign.sigfiles" v-bind:key="index">
                  ...
                  <div class="buttonInside">
                    ...
                    <a
                      v-if="multisign.signature == ''"
                      class="inside"
                      @click="
                        removeFile('mssf', item);
                        getProgress();
                      "
                      ><img src="/img/icons/delete.png" style="width: 25px; opacity: 0.2"
                    /></a>
        ...
        <b-col cols="6">
          ...
          <b-row style="margin-bottom: 10px">
            <b-col>
              <b-row
                v-if="
                  this.multisign.signature == '' && multisign.txfile_valid && multisign.progress >= threshold
                "
              >
                <b-col style="text-align: end">
              ...
              <b-row v-if="this.multisign.signature != ''">
                <b-col style="text-align: end">
                  ...
                  <a class="btn btn-primary mt-1 ml-3" @click="broadcastTx">{{ $t('broadcast') }}</a>
                </b-col>
              </b-row>
            </b-col>
          </b-row>
        ...
        <b-row align-v="center">
          ...
          <b-col>
            <div v-cloak @drop.prevent="upload" @dragover.prevent class="upload-area" ref="myFile">
              <p><img src="/img/icons/add.png" style="width: 100px; opacity: 0.2; margin-bottom: 20px" /></p>
...
import { BTable, BRow, BCol, BProgress, BProgressBar } from 'bootstrap-vue';
...
export default {
  ...
  methods: {
    ...
    downloadSig() {
      ...
      let href = 'data:text/plain;charset=utf-8,' + encodeURIComponent(this.multisign.signature);
    ...
    async msignTx() {
      ...
      for (var sig in this.multisign.signed) {
        signatures.push([
          this.multisign.signed[sig].address,
          Buffer.from(Object.values(this.multisign.signed[sig].signature)),
        ]);
      }
      ...
      var b64encoded = btoa(
        String.fromCharCode.apply(null, Uint8Array.from(TxRaw.encode(signedTx).finish()))
      );
        ...
        this.multisign.sigfiles = this.multisign.sigfiles.filter((f) => {
          try {
            this.pubkeys.forEach(
              (key) =>
                (key.status = key.address == this.multisign.signed[file.name][0] ? 'pending...' : key.status)
            );
