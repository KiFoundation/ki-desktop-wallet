<template>
  <b-modal :id="modalId" hide-footer @show="onResetModal">
        ...
        <div id="transfer-form" class="tab-pane in active w-100">
          <form class="basic-form">
            ...
            <datalist id="self_wallets">
              <option v-for="item in wallets" :key="item.address" :value="item.address">
                {{ item.account }}
              </option>
            </datalist>
            <ul class="basic-group clearfix">
              ...
              <li class="token">
                ...
                <select v-model="transfer.token">
                  <option v-for="item in availableTokens" :key="item.denom" :value="item.denom">
                    {{ item.name }}
                  </option>
                </select>
              </li>
            ...
            <label>{{ $t('memo') }}</label>
            <input v-model="transfer.memo" type="text" :placeholder="$t('webwallet_memo_pl')" />
            <div v-if="!multisig">
            ...
            <b-row align-v="center">
              <b-col class="d-flex justify-content-center">
                ...
                <span v-else>
                  <a v-if="transfer.output == ''" class="btn btn-primary" @click="sendTransfer"> Generate </a>
                  <a v-else class="btn btn-download" @click="download()"> Download </a>
                </span>
              </b-col>
            </b-row>
          </form>
  ...
  data() {
    return {
      ...
      transfer: {
        ...
        account: '',
        amount: 0,
        token: this.globalData.kichain.udenom,
        memo: '',
        fee: 0.00125,
        gasPrice: '0.0000005',
        gasLimit: 300000,
    ...
    async sendTransfer() {
      ...
      const transaction = {
        ...
        fee: {
          amount: [
            {
              denom: this.globalData.kichain.udenom,
              amount: fee.toString(),
            },
          ],
          gas: limit.toString(),
    ...
    download() {
      ...
      return util.download(
        date_today +
          '_transfer_' +
          this.transfer.amount +
          (this.transfer.token === this.globalData.kichain.udenom
            ? this.globalData.kichain.denom
