            ...
            <div class="mnemonic-form">
              <div v-if="step == 0">
                ...
                <input
                  ...
                  "
                />
                <span v-if="!name_correct" class="mnemonic-error">{{ $t('error_wallet_name') }}</span>
                  ...
                  <b-col style="text-align: right">
                    <b-button-group size="sm">
                      <b-button
                        ...
                        :key="idx"
                        :pressed="filter_offline === btn.filter"
                        variant="outline-primary"
                        style="margin-top: 0px"
                        @click="btn.onPress('offline')"
                      >
                        {{ btn.caption }}
                      </b-button>
                    </b-button-group>
              ...
              <div class="mnemonic-form">
                <b-row v-if="step != 0" style="margin-bottom: 10px">
                        ...
                        <ul>
                          <li>
                            <div
                              ...
                              "
                            >
                              <label style="margin: 0px" :for="'w0'">1.</label>
                              <input
                                autocomplete="off"
                                :id="'w0'"
                                type="text"
                                v-model="mnemonic_array[0]"
                                @input="parseMnemonic"
                              />
                            </div>
                          </li>
                          <li v-for="(item, idx) in new Array(23)" v-bind:key="'w' + (idx + 2)">
                            <div
                              ...
                              "
                            >
                              <label style="margin: 0px" :for="'w' + (idx + 2)">{{ idx + 2 }}.</label>
                              <input
                                autocomplete="off"
                                :id="'w' + (idx + 2)"
                                type="text"
                                v-model="mnemonic_array[idx + 1]"
                  ...
                  <b-col v-if="step == 1 && !multisig && !offline_wallet && import_mode === 'k'">
                    ...
                    <span v-if="!private_key_correct" class="mnemonic-error">{{
                      $t('error_privatekey')
                    }}</span>
                  </b-col>
                  ...
                  <b-col v-if="step == 1 && multisig && !offline_wallet">
                    ...
                    <div v-if="require_ms_data" class="optional-area">
                      ...
                      <b-row style="margin-bottom: 20px" align-v="center">
                        <b-col cols="7"
                          ><label>{{ $t('enter_ms_address_pubkeys_sorted') }}</label></b-col
                        >
                        <b-col style="text-align: right">
                          <b-button-group size="sm" style="margin-top: 32px">
                            <b-button
                              v-for="(btn, idx) in buttons"
                              :key="idx"
                              :pressed="filter_sort === btn.filter"
                              variant="outline-primary"
                              style="margin-top: 0px"
                              @click="btn.onPress('sort')"
                            >
                              {{ btn.caption }}
                            </b-button>
                          </b-button-group>
                    ...
                    <div class="buttonInside">
                      ...
                      <a class="inside" @click="toggletPasswordVisible">
                        <img
                          ...
                          src="/img/icons/eye-off.png"
                          style="width: 25px; opacity: 0.2"
                        />
                        <img v-else src="/img/icons/eye-on.png" style="width: 25px; opacity: 0.2" />
                      </a>
                    </div>
                  </b-col>
                </b-row>
...
<script lang="ts">
import { BRow, BCol, BButton, BButtonGroup } from 'bootstrap-vue';
...
export default {
  ...
  methods: {
    ...
    bech32ToAdd(publickeyBech32) {
      return Buffer.from(
        bech32.fromWords(bech32.decode(createAddress(publickeyBech32, 'ki')).words)
      ).toString('hex');
    },
    ...
    validateAddress() {
      if (!this.offline_wallet) {
        if (!this.ms_address) {
          ...
          if (
            !this.ms_address.startsWith(this.prefix + '1') ||
            this.ms_address.length != 39 + this.prefix.length
          ) {
            this.ms_address_correct = false;
          } else {
        ...
        if (!this.offline_address) {
          ...
          if (
            !this.offline_address.startsWith(this.prefix + '1') ||
            this.offline_address.length != 39 + this.prefix.length
          ) {
            this.offline_address_correct = false;
          } else {
