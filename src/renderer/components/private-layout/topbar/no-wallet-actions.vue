<template>
  <div class="d-flex w-100 h-100 justify-content-between align-items-center">
    ...
    <div class="d-flex flex-row">
      <div class="pr-4">
        ...
        >
          ...
          <span class="mt-2" :style="{ fontWeight: '500' }"> Create a Wallet </span>
        </a>
      </div>
      <div class="border-left pl-4">
        ...
        >
          ...
          <span class="mt-2" :style="{ fontWeight: '500' }"> Import a Wallet </span>
        </a>
      </div>
    </div>
    <!-- =======================Import modal============================= -->
    ...
    />
    <!-- =======================Create modal============================= -->
    <CreateWalletForm modalId="add-form-topbar" @onImportCreatedWallet="handleImportWallet" />
  </div>
</template>
  ...
  methods: {
    ...
    async getTotalBalance() {
      ...
      for (var w in this.wallets) {
        const responseBalances = await services.wallet.fetchBalancesList(this.wallets[w].address);

        if (responseBalances.data.result.length > 0) {
          const balanceXKI = responseBalances.data.result.find(
            (b) => b.denom === import.meta.env.VITE_KICHAIN_UDENOM
          );
          const balanceUSDC = responseBalances.data.result.find(
            (b) => b.denom === import.meta.env.VITE_USDC_DENOM
          );

          if (balanceXKI) {
            total += parseInt(balanceXKI.amount);
          }
          if (balanceUSDC) {
            totalUSDC += parseInt(balanceUSDC.amount);
          }
        }
