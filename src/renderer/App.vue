...
import { FETCH_VALIDATORS_LIST, FETCH_VALIDATORS_DICT } from '@store/validators';
    ...
    getAccounts() {
      return new Promise((res) => {
        if (localStorage.getItem('wallet_list')) {
          const wallets = [];
          const wallets_dict = {};
          let wallet_list = localStorage.getItem('wallet_list')
            ...
            if (localStorage.getItem(wallet_list[w])) {
              ...
              if (valid) {
                var wallet_tmp = {
                  ...
                  category:
                    lse_temp.category != undefined && this.categories.includes(lse_temp.category)
                      ? lse_temp.category
                      : 'uncategorized',
                };
